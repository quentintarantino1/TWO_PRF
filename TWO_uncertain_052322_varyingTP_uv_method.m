...
%     sigma2_theta = sigma2thetaval(indwt,5);
%     sigma2_alpha = sigma2alphaval(5);
...
%             E_0(indkx,indkz,indwt) = E_0(indkx,indkz,indwt);
            ...
            uv_2(:,indkx,indkz,indwt) = real(diag(Cu*invQhalf*X02*invQhalf*Cv'));
             % the 2nd-order correction to U if the pressure gradient is
            % kept constant and the bulk flux is allowed to change
...
for indkx = 1:kxgrd
    ...
    for indkz = 1:kzgrd
        ...
        U2_UBconst_nuT2_pre(:,indkx,indkz,:) = kx*kz*U2_UBconst_nuT2(:,indkx,indkz,:);
...
for indwt = 1:wtgrd
    for indy = 1:N
        k_2_avg(indy,indwt)       = sum(sum(k_2_pre(indy,:,:,indwt),3),2)*logkxkz_scale;
        epsilon_2_avg(indy,indwt) = sum(sum(epsilon_2_pre(indy,:,:,indwt),3),2)*logkxkz_scale;
        nuT_2_avg(indy,indwt)     = sum(sum(nuT_2_pre(indy,:,:,indwt),3),2)*logkxkz_scale;
        U2_nuT2_avg(indy,indwt)   = sum(sum(U2_nuT2_pre(indy,:,:,indwt),3),2)*logkxkz_scale;
        U2_UBconst_nuT2_avg(indy,indwt) = sum(sum(U2_UBconst_nuT2(indy,:,:,indwt),2),3)*logkxkz_scale;
        
        U_2_UBconst_avg(indy,indwt) = sum(sum(U_2_UBconst_pre(indy,:,:,indwt),2),3)*logkxkz_scale;
%         UB_nuT2_avg(indwt) = sum(sum(UB_nuT2_pre(indy,:,indwt),2),1)*logkxkz_scale;
%         Px_nuT2_avg(indy,indwt) = sum(sum(Px_nuT2_pre(indy,:,indwt),2),1)*logkxkz_scale;
    end
...
yp = R*(1-y(1:(end-1)/2));
figure;
plot(yp,nuT_2_avg(1:(end-1)/2,5));
set(gca,'fontname','cmr10','FontSize',18);
set(gca,'Xscale','log')

figure;
plot(yp,U2_UBconst_nuT2_avg(1:(end-1)/2,5));
set(gca,'fontname','cmr10','FontSize',18);
set(gca,'Xscale','log')
